...
function solution(scores) {
    ...
    function getScore() {
        const result = [];
        const stack = scores.reduce((result, row) =>
            row.map((_, idx) => {
                return [...(result[idx] || []), row[idx]]
            }), []
        );

        stack.forEach((score, idx) => {
            let total = score.reduce((total, num) => total + num);
            let division = score.length;

            const max = Math.max(...score);
            const min = Math.min(...score);
            if (score[idx] === max || score[idx] === min) {
                if (score.indexOf(score[idx]) === score.lastIndexOf(score[idx])) {
                    total -= score[idx];
                    division--;
                }
            }

            result.push(total / division);
        })
