...
function solution(table, languages, preference) {
    return Array.from(Object.entries(
        table
            ...
            .reduce((result, current) => {
                const [key, ...state] = current.split(' ');
                const tableInfo = {};
                state.forEach((lang, idx) => tableInfo[lang] = (state.length - idx));

                const score = languages.reduce((result, lang, idx) => result + preference[idx] * (tableInfo[lang] || 0), 0);
                if (result.max <= score) {
                    result.max = score;
                    result[key] = score;
                }
    ...
    )).reduce((result, current) => current[1] >= result[1] ? current : result)[0];
}
