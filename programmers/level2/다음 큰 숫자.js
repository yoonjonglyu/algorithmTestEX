...
function solution(n) {
    const check = Array.from(n.toString(2)).filter((char) => char === "1").length;
    for (let int = n + 1; true; int++) {
        if (check === Array.from(int.toString(2)).filter((char) => char === "1").length) {
            return int;
        }
    }
}

/* 다른 사람 풀이 비트 연산, 높은 효율성 */
function solution(n) {
    var i, j;
