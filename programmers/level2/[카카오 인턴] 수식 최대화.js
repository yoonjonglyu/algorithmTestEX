...
function solution(expression) {
    ...
    const expOp = expression.replace(/[0-9]/g, '').split('');
    ...
    operator.forEach((_) => {
        ...
        const count = (operator) => {
            ...
            const opArr = Array.from(expOp);

            while (numArr.length > 1) {
                operator.forEach((current) => {
                    for (let idx = 1; idx < numArr.length; idx++) {
                        if (opArr[idx - 1] === current) {
                            numArr[idx - 1] = calculation(numArr[idx - 1], numArr[idx], current);
                            numArr.splice(idx, 1);
                            opArr.splice(idx - 1, 1);
                            idx--;
                        }
                    }
                });
        ...
        count(operator);
        count(Array.from(operator).reverse());
    });
